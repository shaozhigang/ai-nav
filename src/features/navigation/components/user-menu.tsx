...
import { IconUser, IconLogout, IconLayoutDashboard, IconShield } from "@tabler/icons-react";
          ...
          <div className="absolute right-0 mt-2 min-w-[12rem] max-w-[18rem] bg-popover rounded-lg shadow-navbar border border-border py-1 z-20">
            ...
            {isAdmin && (
              <Link
                href={`/${locale}/admin`}
                onClick={() => setIsOpen(false)}
                className="flex items-center gap-3 px-4 py-2 text-sm text-muted-foreground hover:bg-hover transition-colors whitespace-nowrap"
              >
