...
export async function POST(request: NextRequest) {
  try {
    ...
    // Remove any existing password reset tokens for the user
    await db
      .delete(verification)
      .where(
        and(
          eq(verification.identifier, `${VERIFICATION_TYPE}:${foundUser.id}`),
        )
      );
    ...
    await db.insert(verification).values({
      ...
      identifier: `${VERIFICATION_TYPE}:${foundUser.id}`,
      value: hashedToken,
      expiresAt,
    });
